...
export default function SecUsuarios() {
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <button
          className={`px-3 py-1 text-sm rounded text-white bg-indigo-600 hover:bg-indigo-700 transition ${
            row.original.valido_ate > habilitar_validade ? 'opacity-50 cursor-not-allowed' : ''
          }`}
