...
# Consultas da secretaria (para coordenador)
@app.route('/api/cod_consulta', methods=['GET'])
@login_required
def api_cod_consulta():
...
# Chart: usuários por cargo
@app.route('/api/cod_chart/usuarios_por_cargo', methods=['GET'])
@login_required
...
# Chart: usuários por grupo
@app.route('/api/cod_chart/usuarios_por_grupo', methods=['GET'])
@login_required
def api_usuarios_por_grupo():
    status = request.args.get('status')
    status = None if status == "0" else (True if status == "1" else False)

    grupos = db.session.query(Usuarios.id_grupo, func.count(Usuarios.id_usuario)).group_by(Usuarios.id_grupo).all()
